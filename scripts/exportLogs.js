 ...
 */
export function exportLogsToJSON(outputPath) {
    const logs = getLogs();
 ...
 */
export function exportLogsToCSV(outputPath) {
    const logs = getLogs();
    // CSV header
 ...
 */
export function exportRegistryWithProvenance(outputPath) {
    const registry = loadRegistry();
    const logs = getLogs();
    // Find deployment-related logs
    const deploymentLogs = logs.filter(log => log.eventType.includes('deploy') ||
