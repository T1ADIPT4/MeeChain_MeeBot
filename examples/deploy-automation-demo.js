...
async function demo5_ExportLogs() {
    ...
    // Cleanup demo files
    setTimeout(() => {
        if (fs.existsSync('demo-logs.json'))
            fs.unlinkSync('demo-logs.json');
        if (fs.existsSync('demo-registry-provenance.json'))
            fs.unlinkSync('demo-registry-provenance.json');
    }, 1000);
}
/**
...
// Run all demos
runAllDemos();
//# sourceMappingURL=deploy-automation-demo.js.map