 ...
 */
export async function verifyQuestConditions(userId, questId) {
    try {
        logEvent('quest-verification-start', { userId, questId }, 'debug');
        // Get quest definition
        const quest = questDatabase[questId];
        if (!quest) {
            logEvent('quest-not-found', { userId, questId }, 'warn');
            return false;
        }
        // Get user progress
        ...
        for (const condition of quest.conditions) {
            ...
            if (userCompleted < condition.required) {
                ...
                logEvent('quest-condition-not-met', {
                    userId,
                    questId,
        ...
        logEvent('quest-verification-error', { userId, questId, error: String(error) }, 'error');
        return false;
    }
}
/**
 ...
 */
export function updateUserProgress(userId, questId, conditionType, increment = 1) {
 ...
 */
export function getUserProgress(userId, questId) {
    const key = `${userId}-${questId}`;
    return userProgressDatabase[key] || {};
}
//# sourceMappingURL=questVerifier.js.map